...
export type SuppliedDecision={fixtureOwner:"TEST";decisionId:string;requestId:string;evaluationTimeMs:number;canonicalEnvelope:string;transientSeal:string;controls:Readonly<Record<string,boolean>>;outcome:"AUTHORIZATION_APPROVED_PROJECTED"};
export type Projection={requestId:string;evaluationTimeMs:number;gateStates:readonly {gate:string;state:GateState}[];readiness:"PROJECTION_ONLY";canonical:string;transientSeal:string;operationalCounters:{created:0;issued:0;activated:0;suspended:0;revoked:0;renewed:0;delegated:0;transferred:0;persisted:0;requests:0;bundles:0;workspaces:0;plans:0;scenarios:0;captures:0;results:0;evidence:0;candidate:0;adapter:0;network:0;process:0;stage:0;commit:0;tag:0;push:0}};
...
const MIN_EVALUATION_TIME_MS=-62167219200000;
const MAX_EVALUATION_TIME_MS=253402300799999;
const EPOCH_ABSOLUTE_DAYS=719528;
const MONTH_START_DAYS=[0,31,59,90,120,151,181,212,243,273,304,334] as const;
const lifecycleTimestamp=/^([0-9]{4})-([0-9]{2})-([0-9]{2})T([0-9]{2}):([0-9]{2}):([0-9]{2})(?:\.([0-9]{3}))?Z$/;
const isLeapYear=(year:number):boolean=>year%4===0&&(year%100!==0||year%400===0);
function validateEvaluationTimeMs(value:unknown):number{if(typeof value!=="number")fail("EVALUATION_TIME_INVALID");const n=value as number;if(!Number.isFinite(n)||!Number.isSafeInteger(n)||Object.is(n,-0)||n<MIN_EVALUATION_TIME_MS||n>MAX_EVALUATION_TIME_MS)fail("EVALUATION_TIME_INVALID");return n}
function parseLifecycleTimestamp(value:unknown):number{if(typeof value!=="string")fail("LIFECYCLE_INVALID");const text=value as string;const candidate=lifecycleTimestamp.exec(text);if(candidate===null)fail("LIFECYCLE_INVALID");const match=candidate as RegExpExecArray;const year=Number(match[1]),month=Number(match[2]),day=Number(match[3]),hour=Number(match[4]),minute=Number(match[5]),second=Number(match[6]),millisecond=match[7]===undefined?0:Number(match[7]);if(month<1||month>12||day<1||hour>23||minute>59||second>59)fail("LIFECYCLE_INVALID");const monthDays=month===2?(isLeapYear(year)?29:28):(month===4||month===6||month===9||month===11?30:31);if(day>monthDays)fail("LIFECYCLE_INVALID");const leapsBeforeYear=Math.floor((year+3)/4)-Math.floor((year+99)/100)+Math.floor((year+399)/400);const daysBeforeYear=365*year+leapsBeforeYear;const daysBeforeMonth=MONTH_START_DAYS[month-1]!+(month>2&&isLeapYear(year)?1:0);const epochDays=daysBeforeYear+daysBeforeMonth+(day-1)-EPOCH_ABSOLUTE_DAYS;const epochMs=((((epochDays*24)+hour)*60+minute)*60+second)*1000+millisecond;if(!Number.isSafeInteger(epochMs)||epochMs<MIN_EVALUATION_TIME_MS||epochMs>MAX_EVALUATION_TIME_MS)fail("LIFECYCLE_INVALID");return epochMs}
...
function validateAuthorizationEnvelopeAt(e:Envelope,now:number):void{
 ...
 const issued=parseLifecycleTimestamp(e.lifecycle.issuedAt),active=parseLifecycleTimestamp(e.lifecycle.activatesAt),expires=parseLifecycleTimestamp(e.lifecycle.expiresAt);
 if(issued>active||active>now)fail("LIFECYCLE_INVALID");
...
export function validateAuthorizationEnvelope(e:Envelope,evaluationTimeMs:number):void{const now=validateEvaluationTimeMs(evaluationTimeMs);validateAuthorizationEnvelopeAt(e,now)}
...
export function projectAuthorizationReadiness(e:Envelope,evaluationTimeMs:number):Projection{const now=validateEvaluationTimeMs(evaluationTimeMs);validateAuthorizationEnvelopeAt(e,now);const base={requestId:e.requestId,evaluationTimeMs:now,gateStates:AUTHORIZATION_GATES.map(gate=>({gate,state:"PROJECTION_ONLY" as const})),readiness:"PROJECTION_ONLY" as const,operationalCounters:zeros()};const canonical=canonicalizeAuthorization(base);return{...base,canonical,transientSeal:transientAuthorizationSeal(canonical)}}
export function createTestOwnedDecision(e:Envelope,evaluationTimeMs:number):SuppliedDecision{const now=validateEvaluationTimeMs(evaluationTimeMs);const canonicalEnvelope=canonicalizeAuthorization(e);return{fixtureOwner:"TEST",decisionId:"TEST-DECISION-"+e.requestId,requestId:e.requestId,evaluationTimeMs:now,canonicalEnvelope,transientSeal:transientAuthorizationSeal(canonicalEnvelope),controls:Object.fromEntries(AUTHORIZATION_GATES.map(g=>[g,true])),outcome:"AUTHORIZATION_APPROVED_PROJECTED"}}
export function evaluateSuppliedDecision(e:Envelope,d:SuppliedDecision,evaluationTimeMs:number):"AUTHORIZATION_APPROVED_PROJECTED"{const now=validateEvaluationTimeMs(evaluationTimeMs);validateAuthorizationEnvelopeAt(e,now);const c=canonicalizeAuthorization(e);if(d.fixtureOwner!=="TEST"||d.requestId!==e.requestId||d.evaluationTimeMs!==now||d.canonicalEnvelope!==c||d.transientSeal!==transientAuthorizationSeal(c))fail("SUPPLIED_DECISION_INVALID");if(AUTHORIZATION_GATES.some(g=>d.controls[g]!==true))fail("CONTROL_MISMATCH");return d.outcome}
