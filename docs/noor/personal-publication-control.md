...
Gate Schema v3 / Hook Policy v5 — Tracked Candidate

> The tracked publication-control candidate is Hook Policy v5. The accepted installed runtime hook remains Hook Policy v4 until a separately authorized post-merge v5 hook installation. The tracked v5 candidate is not live publication authority.
...
The tracked Policy-v5 candidate uses gate schema v3 and hook policy version v5.
All gates generated by the tracked candidate, regardless of operation, use
schema version 3 and hook policy version 5. The installed authoritative runtime
hook remains Hook Policy v4 until a separately authorized post-merge v5 hook
installation; therefore the tracked candidate is not live publication authority.
...
This sequence preserves the current Policy-v4 runtime authority during bootstrap
and prevents a worktree-only Policy-v5 generator from issuing authority that the
installed Policy-v4 hook cannot accept.
...
The tracked Policy-v5 candidate generates new Gates with
`SCHEMA_VERSION=3` and `HOOK_POLICY_VERSION=5` for CREATE, UPDATE, TAG, and
DELETE. Until the separately authorized rollout completes, the accepted
installed runtime remains Hook Policy v4; therefore the worktree Policy-v5
generator is not live authority.
