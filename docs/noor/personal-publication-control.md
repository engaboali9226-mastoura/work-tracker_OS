...
Gate Schema v3 / Hook Policy v5 — Installed Live Runtime

> The accepted installed publication-control runtime is Gate Schema v3 / Hook Policy v5. Policy-v5 installation and live-authority rollout are complete; installed hook SHA-256: `1f0154b7b59e22619487b6d00f652532f7d7a55d4e1aab4067e6bf64cb676e2c`. `active.gate`, `state.lock`, and `consume.lock` are absent after the completed rollout and legacy remote cleanup.
> Final post-cleanup audit state: 125 lines; SHA-256: `82af9050ee66ec9159ea9c8ec5c31d9db16e0deb4558687535ad73c66da64c20`.
...
- This mechanism does **not** create arbitrary tags; an annotated tag is created only through an explicitly authorized `CREATE_ANNOTATED_TAG_EXACT_OBJECT` operation.
...
| `tests/noor-publication/` | Acceptance-test suite (T01–T53) |
...
│   └── pre-push.6e4dd829…                  (Policy-v4 hook backup, preserved after Policy-v5 installation)
├── consumed/                               (mode 0700)
│   ├── dc14eb36845….gate                    (v1 consumed gate, pre-existing)
│   └── <NONCE>.gate                         (historical consumed gate; includes preserved Schema v2 / Policy v2, Schema v2 / Policy v3, and Schema v3 / Policy v4 records)
└── revoked/                                (mode 0700, created at bootstrap)
    └── <NONCE>.gate                         (historical revoked/expired gate; includes preserved Schema v2 / Policy v2 and Schema v3 / Policy v4 records)
```

## Gate Schema v3

The accepted installed Policy-v5 runtime uses gate schema v3 and hook policy version v5.
New live Gates use schema version 3 and hook policy version 5. The separately
authorized Policy-v5 hook installation and live-authority rollout are complete.
The installed authoritative hook is the accepted Policy-v5 runtime.

Historical records from earlier schema/policy versions—including existing
Schema v2 / Policy v2, Schema v2 / Policy v3, and Schema v3 / Policy v4
records—remain immutable historical evidence only; they are not reusable
active authority.
...
| 13 | `HOOK_POLICY_VERSION` | Literal `5` |
...
16. `DESTINATION_REF` is either:
    - `refs/heads/pub/[0-9a-f]{12}`, preserving the existing publication
      namespace semantics; or
    - one exact legacy non-publication ref paired with the exact
      `REQUIRED_REMOTE_OBJECT` below:
    ...
    No wildcard authorization exists for `codex/*`, `agents/*`, or arbitrary
    `refs/heads/*`.
...
23. `rmdir state.lock`
24. **Exit 0** → Git proceeds with the single authorized network delete using `:<DESTINATION_REF>`
...
1. **Implementation candidate:** create the tracked Gate Schema v3 / Hook Policy
   v5 candidate in the working tree. No staging or committing is implied.
2. **Independent review:** review the complete working-tree candidate read-only.
3. **Stage and commit:** only after separate explicit authorization, record the
   accepted candidate in one exact local commit.
4. **Policy-v4 publication bootstrap:** while the installed authoritative hook
   remains Policy v4, publication of the Policy-v5 implementation commit must
   use the separately reviewed and accepted Policy-v4 publication tooling from
   the canonical parent. The uninstalled Policy-v5 worktree generator must not
   be treated as live authority for that bootstrap.
5. **PR and merge:** publication, PR creation/review, and merge each remain
   separately authorized lifecycle steps.
6. **Canonical reconciliation:** reconcile the local canonical branch only after
   the accepted remote merge result is proven.
7. **Policy-v5 hook installation:** install the v5 hook only from the exact
   accepted merged commit and only under a separate explicit authorization.
8. **Policy-v5 live authority:** only after the installed hook identity is
   independently verified as Policy v5 may new Policy-v5 live Gates be created.

This sequence preserved the then-current Policy-v4 runtime authority during the completed bootstrap
and prevented a worktree-only Policy-v5 generator from issuing authority that the
then-installed Policy-v4 hook could not accept.
...
|---|---|---|---|
| v1 hook (PR #4) | SHA `4c6f4814…` | `backups/pre-push.4c6f4814…` | Backed up during the Policy-v4 bootstrap; preserved after Policy-v5 installation. |
...
The accepted installed Policy-v5 runtime generates new Gates with
`SCHEMA_VERSION=3` and `HOOK_POLICY_VERSION=5` for CREATE, UPDATE, TAG, and
DELETE. The separately authorized rollout is complete, so new live publication
authority uses schema v3 / policy v5. The four exact legacy non-publication
delete exceptions remain encoded in Policy-v5 semantics as rollout-era
capability; their corresponding remote refs are now absent and the legacy
remote cleanup lifecycle is closed. Removing that capability requires a
separately governed policy revision rather than silently changing Policy-v5.

Historical consumed gates from earlier policy versions remain immutable
historical evidence only and cannot be reused as authority. After a separately
authorized Policy-v5 installation, new live authority uses schema v3 / policy
v5 only.
